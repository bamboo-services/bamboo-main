...
export function RegularFriendCard({ link, onOpen }: FriendCardProps) {
  ...
  return (
    <a
      ...
      href={link.url}
      onClick={handleClick}
      className="group isolate relative flex items-center gap-3 overflow-hidden rounded-lg border border-border bg-card p-4 transition-all duration-300 hover:-translate-y-0.5 hover:border-[var(--ink-border-hover)] hover:shadow-[0_14px_30px_-22px_oklch(0.32_0.06_155/0.4)]"
      style={
        fancy
          ? {
              background:
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <h3 className="truncate font-serif text-[15px] font-bold text-text-primary transition-colors group-hover:text-[var(--ink-text-hover)]">
          {link.name}
        </h3>
