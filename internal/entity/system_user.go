...
// SystemUser 表示一个系统用户实体，用于管理员与普通用户的管理。
//
// 该类型包含用户的唯一标识符、用户名、密码、邮箱等信息。管理员身份不入库，
// 由 bm_system 配置表中的 system.admin.id 唯一标记，经 IsAdmin 计算字段在返回时填充。
...
type SystemUser struct {
	...
	IsAdmin            bool       `json:"is_admin" gorm:"-"`                                                                // 是否为系统唯一管理员（计算字段，不入库）
