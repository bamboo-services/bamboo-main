...
func TaskRssObtain(ctx context.Context) {
	gtimer.Add(ctx, time.Minute*10, func(_ context.Context) {
		...
		// 获取所有链接的Rss信息
		var getLinkHasRss []*entity.LinkList
		err := dao.LinkList.Ctx(ctx).
			Where(do.LinkList{Status: 1}).WhereNull("deleted_at").WhereNotNull("site_rss_url").
			Scan(&getLinkHasRss)
		if err != nil {
			g.Log().Warning(ctx, "[TASK] 获取链接信息失败，等待下次重试")
		...
		if len(getLinkHasRss) > 0 {
			for _, rssLink := range getLinkHasRss {
				...
				err := dao.LinkRss.Ctx(ctx).Where(do.LinkRss{LinkId: rssLink.Id}).Scan(&getRssInfo)
				if err != nil {
					continue
				}
				...
				// 无法正常获取
				g.Log().Warningf(ctx, "无法正常获取 %v 的 RSS 订阅信息", rssLink.SiteRssUrl)
			}
		}
		g.Log().Noticef(ctx, "[TASK] 操作 RSS 订阅完成, 耗时: %vms", time.Now().UnixMilli()-getNowTimestamp)
	})
}
